...
<div class="max-w-screen-md mx-auto bg-white border border-slate-100 rounded-xl p-6 shadow-sm">
    <div class="mb-6">
        <h1 class="text-xl font-bold text-slate-800">Edit Catatan</h1>
        <p class="text-sm text-slate-500 mt-1">Perbarui catatan Anda.</p>
    </div>
    ...
    <form action="{{ route('notes.update', $note) }}" method="POST" class="space-y-4">
        ...
        <div>
            ...
            <textarea name="content" rows="6" 
                      class="w-full px-4 py-2 border border-slate-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500">{{ old('content', $note->content) }}</textarea>
        </div>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
                ...
                <select name="color" class="w-full px-4 py-2 border border-slate-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500">
                    <option value="bg-blue-500" {{ old('color', $note->color) == 'bg-blue-500' ? 'selected' : '' }}>Biru</option>
                    <option value="bg-green-500" {{ old('color', $note->color) == 'bg-green-500' ? 'selected' : '' }}>Hijau</option>
                    <option value="bg-purple-500" {{ old('color', $note->color) == 'bg-purple-500' ? 'selected' : '' }}>Ungu</option>
                    <option value="bg-orange-400" {{ old('color', $note->color) == 'bg-orange-400' ? 'selected' : '' }}>Oranye</option>
                    <option value="bg-red-500" {{ old('color', $note->color) == 'bg-red-500' ? 'selected' : '' }}>Merah</option>
                    <option value="bg-slate-500" {{ old('color', $note->color) == 'bg-slate-500' ? 'selected' : '' }}>Abu-abu</option>
                </select>
            </div>
            
            <div class="flex items-center mt-6">
                <input type="checkbox" id="pinned" name="pinned" value="1" {{ old('pinned', $note->pinned) ? 'checked' : '' }}
