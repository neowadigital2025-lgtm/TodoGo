...
<div class="max-w-screen-xl mx-auto space-y-6" x-data="{ view: 'list' }">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <div class="flex w-full flex-col gap-2 sm:w-auto sm:flex-row sm:items-center sm:gap-5">

        <a href="{{ route('tasks.create') }}" class="inline-flex w-full items-center justify-center gap-2 px-4 py-2 bg-blue-600 hover:bg-blue-700
                       text-white text-sm font-medium rounded-lg transition-colors shadow-sm shrink-0 sm:w-auto">
            ...
            @csrf
            @method('DELETE')
                <button type="submit" class="inline-flex w-full items-center justify-center gap-2 rounded-lg bg-red-600 px-4 py-2 text-sm font-medium text-white shadow-sm transition-all hover:bg-red-700 hover:shadow-md sm:w-auto">
            {{-- Icon Trash --}}
    ...
    {{-- ── Filter bar ───────────────────────────────────────────────── --}}
    <form method="GET" action="{{ route('tasks.index') }}" class="grid grid-cols-2 items-stretch gap-3 sm:flex sm:flex-row sm:items-center" id="filter-form">

        {{-- Search --}}
        <div class="relative col-span-2 w-full sm:max-w-xs sm:flex-1">
        ...
        {{-- Priority filter --}}
        <select name="priority" class="w-full min-w-0 px-3 py-2 text-sm bg-white border border-slate-200 rounded-lg
        ...
        {{-- Status filter --}}
        <select name="status" class="w-full min-w-0 px-3 py-2 text-sm bg-white border border-slate-200 rounded-lg
                       focus:outline-none focus:ring-2 focus:ring-blue-500 text-slate-600"
                onchange="document.getElementById('filter-form').submit()">
        ...
        {{-- Tenggat filter --}}
        <select name="filter" class="w-full min-w-0 px-3 py-2 text-sm bg-white border border-slate-200 rounded-lg
                       focus:outline-none focus:ring-2 focus:ring-blue-500 text-slate-600"
                onchange="document.getElementById('filter-form').submit()">
        ...
        @if(request()->hasAny(['search', 'priority', 'status', 'filter']))
            <a href="{{ route('tasks.index') }}" 
               class="col-span-2 px-3 py-2 text-center text-sm text-slate-500 hover:text-slate-700 border border-slate-200
                       rounded-lg hover:bg-slate-50 transition-colors sm:col-span-1">
                Bersihkan
            </a>
        @endif

        {{-- View toggle --}}
        <div class="col-span-2 flex w-full items-center justify-center gap-1 p-1 bg-white border border-slate-200 rounded-lg sm:col-span-1 sm:ml-0 sm:w-auto">
