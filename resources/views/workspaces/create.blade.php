...
@section('title', 'Buat Ruang Kerja')

@section('content')
<div class="max-w-screen-md mx-auto bg-white border border-slate-100 rounded-xl p-6 shadow-sm">
    <div class="mb-6">
        <h1 class="text-xl font-bold text-slate-800">Buat Workspace Baru</h1>
        <p class="text-sm text-slate-500 mt-1">Tambahkan workspace baru untuk tim kamu.</p>
    </div>
    ...
    <form action="{{ route('workspaces.store') }}" method="POST" class="space-y-4">
        ...
        <div>
            ...
            <textarea name="description" rows="3" 
                      class="w-full px-4 py-2 border border-slate-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500">{{ old('description') }}</textarea>
        </div>
